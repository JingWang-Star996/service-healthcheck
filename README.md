# Service Healthcheck — Universal Self-Check Script
...
> A pure Bash script for comprehensive self-check of any long-running service.
...
```bash
git clone https://github.com/JingWang-Star996/service-healthcheck.git
...
Harness（及类似 AI Agent 框架）长时间运行时，常见问题包括：内存泄漏导致 OOM 崩溃、子代理卡死/僵尸进程、代码死循环疯狂写文件、WebSocket 连接泄漏、崩溃日志堆积但无人发现。传统做法是用 AI agent 手动检查——**浪费 token，无法积累趋势**。

### 架构

```
系统 crontab (每小时)        Harness cron (错开几分钟)
      ↓                            ↓
...
```
System crontab (hourly)        Harness cron (offset)
      ↓                            ↓
...
**Harness cron (anomaly notification):**
```json
{
...
```
システム crontab（毎時）        Harness cron（ずらす）
      ↓                            ↓
...
```
시스템 crontab (매시간)        Harness cron (시간차)
      ↓                            ↓
...
```
Crontab del sistema (cada hora)    Cron de Harness (desfasado)
      ↓                                    ↓
...
**Harness cron (notificación de anomalías):**
```json
{
...
```
Crontab système (chaque heure)    Cron Harness (décalé)
      ↓                                ↓
...
**Harness cron (notification d'anomalies) :**
```json
{
...
```
System-Crontab (stündlich)        Harness-Cron (versetzt)
      ↓                                ↓
...
**Harness cron (Anomalie-Benachrichtigung):**
```json
{
  "name": "Prüfergebnis",
  "schedule": { "kind": "every", "everyMs": 3600000 },
...
```
Системный crontab (каждый час)    Harness cron (со сдвигом)
      ↓                                ↓
...
**Harness cron (уведомление об аномалиях):**
```json
{

...
=============================================
  OpenClaw 自检报告 — 2026-05-01 11:14:55 CST
=============================================
...
{"ts":"2026-05-01T11:14:38+08:00","check":"L2-01","metric":"status","value":0,"status":"ok"}
```

### 4. 异常标记文件

有异常时创建 `/tmp/openclaw/health-anomaly`，供上层系统快速判断。
...
```json
{
  ...
  "payload": {
    "kind": "agentTurn",
    "message": "读取 data/self-check-trends/status.json，检查 overall 字段。如果 ok → NO_REPLY。否则提取 anomalies 私聊通知。"
  },
...
```markdown
每次 Heartbeat 执行:
bash /path/to/scripts/healthcheck.sh
脚本输出中看 🔴/🚨 标记，有异常则汇报。
```
